    ...
    def longestConsecutive(self, nums: list[int]) -> int:
        ...
        for number in nums:
            ...
            if number - 1 not in num_set:
                ...
                length = 1
                # Use a while loop to extend the current sequence. While 'number + length' is present in 'num_set', increment 'length'. 
                ...
                while number + length in num_set:
                    length += 1
                # After determining the length of the current sequence, update 'longest' by taking the maximum of 'longest' and 'length'. 
                # This ensures that 'longest' always holds the length of the longest consecutive sequence found so far.
...
for i, (nums, expected) in enumerate(test_cases):
    result = solution1.longestConsecutive(nums)
...
class Solution2:
    def longestConsecutive(self, nums: list[int]) -> int:
        ...
        # Iterate through each 'number' in the list 'nums' 
        for number in nums:
            # If the number is not part of any sequence.
...
solution2 = Solution2()
for i, (nums, expected) in enumerate(test_cases):
    result = solution2.longestConsecutive(nums)
    print(f"Test Case {i + 1}: {'Pass' if result == expected else 'Fail'} (Expected {expected}, Got {result})")